...
expect.extend({
    toBeSuccess(received: ParserResult<any>) : jest.CustomMatcherResult {
        const pass : boolean = received.success && received.remaining.available() === 0;
    ...
    toHaveRemaining(received: ParserResult<any>, remaining: string) : jest.CustomMatcherResult {
        const pass : boolean = received.success && received.remaining.take(received.remaining.available()) === remaining;
        const message: () => string = () => {
            if(received.success) {
                const actualRemaining = received.remaining.take(received.remaining.available());
