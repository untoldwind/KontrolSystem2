...
import { recognizeAs, map, value, opt, recognize, where } from "../parser/combinator";
import { chars0, charsExcept1, digits0, digits1, oneOf, tag } from "../parser/complete";
import { many0 } from "../parser/multi";
import { between, preceded, seq, terminated } from "../parser/sequence";
import { LiteralBool, LiteralFloat, LiteralInt, LiteralString } from "./ast/literal";
...
export const literalInt = map(
    seq([opt(tag("-")), basePrefix, recognize(terminated(digits1, chars0(ch => isDigit(ch) || ch === UNDERSCORE)))]),
    ([negSign, radix, digits], start, end) => new LiteralInt(
        negSign ? -parseInt(digits.replaceAll("_", ""), radix) : parseInt(digits.replaceAll("_", ""), radix), start, end)
)

const exponentSuffix = seq([oneOf("eE"), opt(oneOf("-+")), digits1]);

export const literalFloat = map(
    recognize(preceded(opt(oneOf("-+")), alt([
...
export const literalBool = alt([
    map(where(identifier, str => str === "true", "true"), (_, start, end) => new LiteralBool(true, start, end)),
    map(where(identifier, str => str === "false", "false"), (_, start, end) => new LiteralBool(false, start, end)),
]);
