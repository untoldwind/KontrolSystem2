...
export function char(predicate: (charCode: number) => boolean, expected: string): Parser<string> {
    return (input: Input) => {
        if (input.available() < 1) return new ParserFailure(expected);
        const current = input.take(1);
        if (!predicate(current.charCodeAt(0))) return new ParserFailure(expected);
...
export function chars0(predicate: (charCode: number) => boolean): Parser<string> {
    return (input: Input) => {
...
export function chars1(predicate: (charCode: number) => boolean, expected: string) : Parser<string> {
    return (input: Input) => {
        let count = input.findNext(ch => !predicate(ch));
        if(count < 0) count = input.available();
        if(count === 0) return new ParserFailure(expected);
        return new ParserSuccess(input.advance(count), input.take(count));
    }
}
...
export function charsExcept1(forbidden: string) : Parser<string> {
    ...
    return chars1(ch => forbiddenCodes.indexOf(ch) < 0, `any character except ${forbidden}`);
}

const ADDITIONAL_WHITESPACE_CHARS = toCharCodes("\n\t\r\f\v");

export const whitespace0 = chars0(ch => isWhiteSpace(ch) || ADDITIONAL_WHITESPACE_CHARS.indexOf(ch) >= 0);

export const whitespace1 = chars1(ch => isWhiteSpace(ch) || ADDITIONAL_WHITESPACE_CHARS.indexOf(ch) >= 0, "<whitespace>");
...
export function tag(tag: string): Parser<string> {
    return (input: Input) => {
        if (input.available() < tag.length) return new ParserFailure(tag);
