...
export function fold0<T, S>(initial: Parser<T>, suffix: Parser<S>, combine: (collect: T, current: S, start: Position, end: Position) => T) : Parser<T> {
    return (input: Input) => {
        ...
        while(suffixResult.success) {
            ...
            result = new ParserSuccess(suffixResult.remaining, combine(result.result, suffixResult.result, result.remaining.position, suffixResult.remaining.position));
...
export function chain<T, OP>(operantParser : Parser<T>, opParser: Parser<OP>, apply: (left: T, op: OP, right: T, start: Position, end: Position) => T) {
    ...
    return (input: Input) => {
        ...
        while (restResult.success) {
            ...
            result = apply(result, op, operant, remaining.position, restResult.remaining.position);
