    ...
    public override REPLValueFuture Eval(REPLContext context) {
        ...
        if (!fieldAccess.FieldType.IsAssignableFrom(context.replModuleContext, valueFuture.Type))
            throw new REPLException(this,
                $"Type '{targetFuture.Type.Name}' field '{fieldName}' is of type {fieldAccess.FieldType} but is assigned to {valueFuture.Type}");
        ...
        var operatorEmitter = fieldAccess.FieldType.AllowedSuffixOperators(context.replModuleContext)
            .GetMatching(context.replModuleContext, op, valueFuture.Type);

        if (operatorEmitter == null)
            throw new REPLException(this,
                $"Type '{targetFuture.Type.Name}' field '{fieldName}': Cannot {op} a {fieldAccess.FieldType} with a {valueFuture.Type}");
