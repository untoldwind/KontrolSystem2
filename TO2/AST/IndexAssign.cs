...
public class IndexAssign : Expression {
    ...
    public override REPLValueFuture Eval(REPLContext context) {
        ...
        if (!indexAccess.TargetType.IsAssignableFrom(context.replModuleContext, valueFuture.Type))
            throw new REPLException(this,
                $"Type '{targetFuture.Type.Name}' element is of type {indexAccess.TargetType} but is assigned to {valueFuture.Type}");
        ...
        var operatorEmitter = indexAccess.TargetType.AllowedSuffixOperators(context.replModuleContext)
            .GetMatching(context.replModuleContext, op, valueFuture.Type);

        if (operatorEmitter == null)
            throw new REPLException(this,
                $"Index assign '{targetFuture.Type.Name}'.'{indexSpec}': Cannot {op} a {indexAccess.TargetType} with a {valueFuture.Type}");
