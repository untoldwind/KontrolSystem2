...
public partial class KSPControlModule {
    ...
    public class SteeringManagerLegacy : IKSPAutopilot {
        ...
        private readonly TorquePI pitchPI = new();

        private readonly PidLoop pitchRatePI = new(1, 0.1, 0, extraUnwind: true);

        private readonly MovingAverage pitchTorqueCalc = new() { SampleLimit = 15 };
        private readonly TorquePI rollPI = new();
        ...
        private readonly PidLoop yawRatePI = new(1, 0.1, 0, extraUnwind: true);
        private readonly MovingAverage yawTorqueCalc = new() { SampleLimit = 15 };
        ...
        */
        ...
        [KSMethod]
        public void Resume() {
            context.HookAutopilot(vessel.vessel, this);
        }
        ...
        public void UpdateControlParts() {
            if (vessel.vessel.SimulationObject.PartOwner.PartCount != vesselParts) {
                ...
                foreach (var part in vessel.vessel.SimulationObject.PartOwner.Parts)
                    foreach (var pm in part.Modules.Values) {
                        var tp = pm as ITorqueProvider;
                        if (tp != null) torqueProviders.Add(pm, tp);
                    }
            }
        }
        ...
        public Vector3 FindMoI() {
            ...
            var partTensor = Matrix4x4.identity;
            var inertiaMatrix = Matrix4x4.identity;
            var productMatrix = Matrix4x4.identity;
            foreach (var part in vessel.vessel.SimulationObject.PartOwner.Parts)
                if (part.SimulationObject.Rigidbody != null) {
                    ...
                    var invMatrix = Matrix4x4.TRS(Vector3.zero, inv, Vector3.one);
                    ...
                    KSPUtil.ToDiagonalMatrix2(part.SimulationObject.Rigidbody.mass * position.sqrMagnitude,
                        ref inertiaMatrix);
        ...
        public void UpdateControl(FlightCtrlState c) {
            if (vessel.vessel.Autopilot.Enabled) {
                ...
                var clampAccYaw = Math.Max(Math.Abs(accYaw), 0.005) * 2;
                accYaw = tgtYawTorque / controlTorque.z;
                if (Math.Abs(accYaw) < Epsilon)
                    accYaw = 0;
                accYaw = Math.Max(Math.Min(accYaw, clampAccYaw), -clampAccYaw);
                c.yaw = (float)accYaw;
                var clampAccRoll = Math.Max(Math.Abs(accRoll), 0.005) * 2;
