...
public static class REPLExpression {
    /// <summary>
    ///     Submits an expression to be evaluated in the console window. The expression and its result will not be printed to
    ///     the console window.
    /// </summary>
        ...
        try {
            ...
            foreach (var item in result.Where(i => !(i is IBlockItem))) {
                ...
                while (!futureResult.IsReady) {
                    pollCount++;
                    if (pollCount > 100) throw new Exception("No result after 100 tries");
