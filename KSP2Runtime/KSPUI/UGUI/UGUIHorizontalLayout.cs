...
public class UGUIHorizontalLayout(RectTransform containerTransform, float gap = 10, UGUILayout.Padding padding = default) : UGUILayout(
    ...
    public override Vector2 MinSize {
        get {
            ...
            if (layoutEntries.Count > 1) minSize.x += (layoutEntries.Count - 1) * gap;
    ...
    public override Vector2 Layout() {
        ...
        var availableWidth = parentRect.width - padding.left - padding.right -
                             layoutEntries.Where(entry => entry.Stretch == 0).Select(entry => entry.MinSize.x).Sum();
        ...
        foreach (var entry in layoutEntries) entry.Layout();
