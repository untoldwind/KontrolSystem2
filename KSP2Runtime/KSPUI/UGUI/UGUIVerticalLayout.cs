...
public class UGUIVerticalLayout : UGUILayout {
    ...
    public override Vector2 MinSize {
        get {
            ...
            if (layoutEntries.Count > 1) minSize.y += (layoutEntries.Count - 1) * gap;
    ...
    public override Vector2 Layout() {
        ...
        var availableHeight = parentRect.height - padding.top - padding.bottom -
                              layoutEntries.Where(entry => entry.Stretch == 0).Select(entry => entry.MinSize.y).Sum();
        if (layoutEntries.Count > 1) availableHeight -= (layoutEntries.Count - 1) * gap;
