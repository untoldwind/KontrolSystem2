...
public partial class KSPConsoleModule {
    ...
    public class Console {
        [KSField] public long CursorRow => KSPContext.CurrentContext.ConsoleBuffer?.CursorRow ?? 0;

        [KSField] public long CursorCol => KSPContext.CurrentContext.ConsoleBuffer?.CursorCol ?? 0;

        [KSMethod(
            Description = "Clear the console of all its content and move cursor to (0, 0)."
        )]
        public void Clear() {
            KSPContext.CurrentContext.ConsoleBuffer?.Clear();
        }
        ...
        public void PrintLine(string message) {
            KSPContext.CurrentContext.ConsoleBuffer?.PrintLine(message);
        }
        ...
        public void ClearLine(long row) {
            KSPContext.CurrentContext.ConsoleBuffer?.ClearLine((int)row);
        }
    }
}
