...
namespace KontrolSystem.KSP2.Runtime.KSPVessel {
    public static class IsPartExtensions {
        public static bool IsPartDeployable(this IDeltaVPart PartRef, out Data_Deployable deployable) {
            ...
            return PartRef.TryGetModuleData<PartComponentModule_Deployable, Data_Deployable>(out deployable) // this doesn't work on PartComponentModule_SolarPanel, unexpectedly...
                || PartRef.TryGetModuleData<PartComponentModule_SolarPanel, Data_Deployable>(out deployable); // ...so we do a call for PartComponentModule_SolarPanel specifically
        }
