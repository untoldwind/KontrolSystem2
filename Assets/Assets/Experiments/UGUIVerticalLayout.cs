...
namespace Experiments {
    public class UGUIVerticalLayout : UGUILayout {
        ...
        public override Vector2 Layout() {
            ...
            var sumStrech = layoutEntries.Select(entry => entry.Strech).Sum();
            parentRect = containerTransform.rect;
            var topAnchor = 1f;
            var offset = parentRect.height - padding.top + 0.5f * gap;
            var availableHeight = parentRect.height - padding.top - padding.bottom - layoutEntries.Where(entry => entry.Strech == 0).Select(entry => entry.MinSize.y).Sum();
            if (layoutEntries.Count > 1) {
                availableHeight -= (layoutEntries.Count - 1) * gap;
            }

            foreach (var entry in layoutEntries) {
                var relativeStretch = entry.Strech > 0 ? entry.Strech / sumStrech : 0;
